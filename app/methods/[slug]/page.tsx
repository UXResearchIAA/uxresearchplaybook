...
import { SOPProgressProvider } from '@/contexts/SOPProgressContext';
...
export default async function MethodDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Build a stepId → checklist count map for the progress context
  const stepChecklistCounts: Record<number, number> = {};
  for (const step of sopSteps) {
    stepChecklistCounts[step.id] = step.checklist.length;
  }
  ...
  return (
    <div>
      ...
      {/* Main: sidebar + content — wrapped in SOPProgressProvider */}
      <SOPProgressProvider slug={method.slug} stepChecklistCounts={stepChecklistCounts}>
